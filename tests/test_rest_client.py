...
class TestJesseRESTClientInit:
    ...
    @patch("jesse_mcp.core.rest.auth.requests.post")
    ...
    def test_init_no_credentials(self, mock_session_class):
        ...
        with patch("jesse_mcp.core.rest.client.get_rate_limiter") as mock_limiter:
            ...
            with patch("jesse_mcp.core.rest.client.logger") as mock_logger:
                client = JesseRESTClient(
                    base_url="http://test:8000",
                    password="",
                    api_token="",
...
class TestAuthenticateWithPassword:
    """Tests for authenticate_with_password function"""
    ...
    def test_authenticate_success(self, mock_post):
        ...
        result = auth.authenticate_with_password(
            mock_session, "http://test:8000", "secret-password"
        )

        assert result == "session-abc-123"
        assert mock_session.headers["authorization"] == "session-abc-123"
    ...
    def test_authenticate_connection_error(self, mock_post):
        ...
        with patch("jesse_mcp.core.rest.auth.logger") as mock_logger:
            with pytest.raises(requests.exceptions.ConnectionError):
                auth.authenticate_with_password(mock_session, "http://test:8000", "password")

            mock_logger.error.assert_called()


class TestAuthenticateWithToken:
    """Tests for authenticate_with_token function"""
    ...
    def test_authenticate_with_token_success(self):
        ...
        result = auth.authenticate_with_token(mock_session, "http://test:8000", "my-api-token-123")

        assert result == "my-api-token-123"
        assert mock_session.headers["authorization"] == "my-api-token-123"
...
class TestVerifyConnection:
    ...
    def test_verify_connection_success(self):
        ...
        with patch("jesse_mcp.core.rest.auth.logger"):
            result = auth.verify_connection(mock_session, "http://test:8000")

        assert result is True
        mock_session.get.assert_called_once_with("http://test:8000/")
    ...
    def test_verify_connection_unauthorized(self):
        ...
        with pytest.raises(ConnectionError, match="Unauthorized"):
            auth.verify_connection(mock_session, "http://test:8000")
...
class TestBacktest:
    ...
    def test_backtest_rate_limited(self):
        ...
        from jesse_mcp.core.rest import JesseRESTClient
        from jesse_mcp.core.rest import backtest as bt_module
        ...
        with patch("jesse_mcp.core.rest.candles.validate_candle_data", return_value=None):
            with patch.object(bt_module, "execute_backtest") as mock_exec:
                mock_exec.return_value = {"error": "Rate limit exceeded", "success": False}
...
class TestOptimization:
    ...
    def test_optimization_rate_limited(self):
        ...
        from jesse_mcp.core.rest import JesseRESTClient
        from jesse_mcp.core.rest import optimization as opt_module

        client = JesseRESTClient.__new__(JesseRESTClient)
        client.base_url = "http://test:8000"
        client.session = Mock()

        with patch("jesse_mcp.core.rest.optimization.get_rate_limiter") as mock_limiter:
...
class TestGetBacktestSessions:
    ...
    def test_get_backtest_sessions_error(self):
        ...
        with patch("jesse_mcp.core.rest.client.logger"):
            result = client.get_backtest_sessions()
...
class TestCachedBacktest:
    ...
    def test_cached_backtest_cache_disabled(self):
        ...
        with patch("jesse_mcp.core.rest.client.JESSE_CACHE_ENABLED", False):
            with patch("jesse_mcp.core.rest.client.get_rate_limiter") as mock_limiter:
                mock_limiter.return_value = Mock(acquire=Mock(return_value=True))
                with patch("jesse_mcp.core.rest.candles.validate_candle_data", return_value=None):
                    result = client.cached_backtest(
                        routes=[
                            {"strategy": "TestStrategy", "symbol": "BTC-USDT", "timeframe": "1h"}
                        ],
                        start_date="2023-01-01",
                        end_date="2023-12-31",
                    )
    ...
    def test_cached_backtest_cache_hit(self):
        ...
        with patch("jesse_mcp.core.rest.client.JESSE_CACHE_ENABLED", True):
            with patch(
                "jesse_mcp.core.rest.client.get_backtest_cache",
...
class TestGetStrategiesCached:
    ...
    def test_get_strategies_cached_error(self):
        """Test get_strategies_cached returns placeholder when cache disabled"""
        from jesse_mcp.core.rest import JesseRESTClient
...
class TestValidateBacktestResult:
    """Tests for validate_backtest_result function"""
...
class TestBacktestWithRetry:
    ...
    def test_is_retryable_error_timeout(self):
        """Test is_retryable_error identifies timeout"""
        from jesse_mcp.core.rest import backtest

        assert backtest.is_retryable_error("Request timeout after 300s")
        assert backtest.is_retryable_error("timeout")

    def test_is_retryable_error_rate_limit(self):
        """Test is_retryable_error identifies rate limit"""
        from jesse_mcp.core.rest import backtest

        assert backtest.is_retryable_error("Rate limit exceeded")
        assert backtest.is_retryable_error("rate limit")

    def test_is_retryable_error_not_retryable(self):
        """Test is_retryable_error identifies non-retryable errors"""
        from jesse_mcp.core.rest import backtest

        assert not backtest.is_retryable_error("Invalid strategy")
        assert not backtest.is_retryable_error("Authentication failed")


class TestGetJesseRestClient:
    """Tests for get_jesse_rest_client singleton function"""

    @patch("jesse_mcp.core.rest.client.JesseRESTClient")
    def test_get_client_creates_singleton(self, mock_client_class):
        ...
        import jesse_mcp.core.rest.client as module
